...
const DEFAULT_DETAILS: PlanDetails = {
  ...
  kpis: [
    ...
    { label: 'Job Type', value: 'Drainage Clearing', icon: <Icons.RefreshCcw01 size={16} /> },
    { label: 'Driver', value: 'Mohammed Al-Kuwari', icon: <Icons.User01 size={16} /> },
    { label: 'Distance per Site', value: '~0.03 km/site', icon: <Icons.Route size={16} /> },
    { label: 'Total Distance', value: '43 km', icon: <Icons.Route size={16} /> },
    { label: 'Frequency', value: 'Daily', icon: <Icons.Clock size={16} /> },
    { label: 'Total Sites', value: '151', icon: <Icons.MarkerPin01 size={16} /> },
    { label: 'Tanker', value: 'LMV-QA01', icon: <Icons.Truck01 size={16} /> },
    { label: 'Number of Crew', value: '2', icon: <Icons.Users01 size={16} /> },
    { label: 'Total Time', value: '6h 53min', icon: <Icons.Clock size={16} /> },
  ],
};
const DEFAULT_MAP: PlanMapPreview = {
  center: [25.30, 51.49], zoom: 10,
  route: [[25.17, 51.61], [25.22, 51.55], [25.29, 51.53], [25.36, 51.49], [25.43, 51.49]],
  clusters: [{ position: [25.20, 51.55], count: 99 }, [25.29, 51.53].length ? { position: [25.36, 51.49], count: 52 } : { position: [25.36, 51.49], count: 52 }],
  start: [25.43, 51.49], discharge: [25.19, 51.61],
  legend: [{ label: '110 Sites (Low Priority)', color: 'var(--status-warning)' }, { label: '52 Sites (High Priority)', color: 'var(--status-error)' }],
};
...
export function CreatePlanWizard({
  serviceTypes = ['Drainage Clearing', 'Pump Deployment', 'Water Extraction', 'Emergency Repair'],
  ...
  const [draft, setDraft] = React.useState<PlanDraft>({
    title: initial?.title ?? 'Al Wakrah Corniche Plan',
          ...
          <div className="mt-5">
            {step === 0 && (
              <div className="flex max-w-2xl flex-col gap-4">
                ...
                <FieldSelect label="Job Type" icon={<Icons.Droplets02 size={18} />} value={draft.serviceType} options={serviceTypes} onChange={(v) => set('serviceType', v)} />
              ...
              <div className="flex flex-col gap-6">
                <section className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-1 gap-x-8 gap-y-3 md:grid-cols-2">
                    ...
                    <SummaryRow label="Job Type" value={<span className="inline-flex items-center gap-1.5"><Icons.Droplets02 size={14} className="text-[color:var(--chart-accent-purple)]" />{draft.serviceType}</span>} />
