...
export interface PlanMonitoringProps {
  ...
  /** header/column-label overrides (default the waste-domain "Service Type"/"Waste Type"). */
  labels?: { serviceType?: string; wasteType?: string };
  /** icon shown in the Service Type chip (default the waste-domain bin icon). */
...
/*
 * 2026-08-31 role-separation fix (user-directed, round-3 corrective): these
 * four tones/colors used to read `var(--primary)` directly. That is correct
 * for brand/ACTION chrome (buttons, active tabs/underlines, selection
 * borders — see `create-plan-wizard.tsx`/`planning-mode.tsx`'s own "Brand
 * law: ... maps to `--primary`" comments), but ONGOING/night-shift/the
 * driver avatar/the "On Going Plans" KPI tile are genuine STATUS/identity
 * semantics, not brand chrome — under the reference's original FAMS-blue
 * theme `--primary` visually doubled as an "info/active" accent, but tying
 * them to `--primary` conflates the two roles the moment a tenant's brand
 * primary is NOT blue (qatar-mme's is deep maroon). `--status-info` is the
 * theme's dedicated, tenant-STABLE "info/active" role token (declared once
 * in `tokens/theme.css`, never overridden by a tenant theme file — see the
 * diff against `qatar-mme.theme.css`), so these four now read that instead:
 * same blue they always rendered, but for the right reason, and independent
 * of whatever a tenant sets `--primary` to.
 */
...
function shiftGlyph(name: string): { icon: React.ReactNode; color: string } {
  const n = name.toLowerCase();
  if (n.includes('night')) return { icon: <Icons.Moon01 size={14} />, color: 'var(--status-info)' };
...
/**
 * Deterministic name → one of a fixed multi-hue accent set (djb2-ish hash,
 * same shape as the DS's own `hashToPaletteClass` in
 * `ui-kit/src/primitives/Avatar.tsx`, reimplemented locally since this
 * vendored app pins React 18 and can't import the React-19 DS package).
 * Gives each driver a STABLE but VARIED identity color instead of every
 * avatar rendering the tenant's brand primary — the same "avatars use a
 * varied palette, not the brand action color" role separation as the status
 * pill above. Drawn from the theme's existing `--chart-accent-*` hues
 * (already loaded, no new tokens), skipping red/orange/green (reserved for
 * error/warning/success elsewhere in this screen) so an avatar color is
 * never mistaken for a status color.
 */
...
function WorkerAvatar({ name, color, size = 24 }: { name: string; color?: string; size?: number }) {
  const initials = name.split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase();
  const resolvedColor = color ?? hashAvatarColor(name);
  return <span className="grid shrink-0 place-items-center rounded-full font-semibold text-white" style={{ width: size, height: size, background: resolvedColor, fontSize: Math.max(12, size * 0.4) }}>{initials || '—'}</span>;
}
...
function defaultKpis(rows: PlanMonitorRow[]): PlanMonitorKpi[] {
  ...
  return [
    ...
    { label: 'On Going Plans', value: n('ONGOING'), icon: <Icons.Activity size={20} />, tone: 'var(--status-info)' },
    { label: 'Scheduled Plans', value: n('SCHEDULED'), icon: <Icons.Clock size={20} />, tone: 'var(--status-warning)' },
  ];
}

export function PlanMonitoring({
  rows, kpis, statusStyles, wasteStyles, shiftStyles, labels, serviceIcon, onOpen, className,
}: PlanMonitoringProps) {
  const [query, setQuery] = React.useState('');
  const sStyles = { ...DEFAULT_STATUS, ...statusStyles };
  const kpiCards = kpis ?? defaultKpis(rows);
  const L = { serviceType: 'Service Type', wasteType: 'Waste Type', ...labels };
  ...
  const columns: DataTableColumn<PlanMonitorRow>[] = [
    ...
    { id: 'service', header: L.serviceType, width: '150px', cell: (r) => (
      <span className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-body-sm font-medium" style={{ background: `color-mix(in srgb, ${VIOLET} 12%, transparent)`, color: VIOLET }}>{serviceIcon ?? <Icons.BinCollection size={14} />}{r.serviceType}</span>
    ) },
    { id: 'waste', header: L.wasteType, width: '130px', cell: (r) => { const w = wasteStyles?.[r.wasteType] ?? wasteGlyph(r.wasteType); return <span className="inline-flex items-center gap-1.5 text-body-sm text-foreground"><span style={{ color: w.color }}>{w.icon}</span>{r.wasteType}</span>; } },
    ...
    { id: 'progress', header: 'Progress of Plan', width: '150px', cell: (r) => {
      ...
      return (
        <span className="flex items-center gap-2">
          {/* `bg-info` (→ `--status-info`), not `bg-primary` — a progress fill is
              a status/active indicator, not brand chrome (see the role-separation
              note above `DEFAULT_STATUS`). 100%-complete gets the success green
              instead, matching "Completed" elsewhere on this screen. */}
          <span className="h-1.5 w-20 overflow-hidden rounded-full bg-muted"><span className={cn('block h-full rounded-full', pct >= 100 ? 'bg-success' : 'bg-info')} style={{ width: `${pct}%` }} /></span>
  ...
  return (
    <div className={cn('flex h-full min-h-0 flex-col gap-4 overflow-auto p-6', className)}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button type="button" aria-label="Filter" className="grid size-10 shrink-0 place-items-center rounded-lg border border-border text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><Icons.FilterFunnel02 size={16} /></button>
        <Popover>
