...
export interface LeafletMapProps {
  ...
  zones?: MapZone[];
  /**
   * Geofence circles (drawn with the zones, under the markers) — depot /
   * assembly-point / discharge-point geozones (FM-6354's detail map). Radius
   * in metres; dashed ring by default to read as a *boundary*, not an area.
   */
  circles?: { id: string; center: LatLng; radius: number; color?: string; label?: string; dashed?: boolean }[];
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      for (const c of circles) {
        const color = c.color ?? '#0072D6';
        const ring = L.circle(c.center, {
          radius: c.radius, color, weight: 1.5, dashArray: c.dashed === false ? undefined : '6 6',
          fillColor: color, fillOpacity: 0.08,
        }).addTo(zoneLayer.current);
        if (c.label) ring.bindTooltip(c.label, { direction: 'top' });
      }
    })();
