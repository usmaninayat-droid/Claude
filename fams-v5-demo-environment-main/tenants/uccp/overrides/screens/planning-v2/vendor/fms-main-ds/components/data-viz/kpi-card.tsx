    ...
    return (
      ...
      >
        ...
        <div className="mt-2 flex items-baseline gap-2">
          <span className="text-h5 font-semibold text-foreground tabular-nums">{value}</span>
          {delta ? (
            <span className={cn('inline-flex items-center gap-0.5 text-caption', deltaColor)}>
        ...
        {footer ? <div className="mt-2 text-caption text-muted-foreground">{footer}</div> : null}
      </div>
    );
  }
);
