  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        {field ? (
          // T-088: flex-col + justify-center (not block-stacked) so a consumer
          // overriding `triggerClassName` with a taller fixed height (e.g. `h-14`
          // to sit flush with a sibling FloatingLabelInput/LabeledSelect field)
          // gets the label+value pair vertically centered instead of pinned to
          // the top with dead space below — same idiom as FloatingLabelInput's
          // own `flex-col justify-center` inner wrapper. No visual change at the
          // default (intrinsic) height.
          <button type="button" className={cn('flex w-full flex-col justify-center gap-0.5 rounded-lg border border-border bg-card px-3 py-2 text-left outline-none transition-colors hover:bg-muted/40 focus-visible:ring-2 focus-visible:ring-ring', triggerClassName)}>
            <span className="text-caption font-medium uppercase tracking-wide text-muted-foreground">{field.label}</span>
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between gap-4 px-4 pt-4">
              <div className="flex items-center gap-3">
                <button type="button" aria-label="Previous month" onClick={() => stepMonth(-1)} className="text-muted-foreground hover:text-foreground"><ChevronLeft size={18} /></button>
                <span className="min-w-[84px] text-center text-body-sm font-semibold text-foreground">{MONTHS[view.m]}</span>
                <button type="button" aria-label="Next month" onClick={() => stepMonth(1)} className="text-muted-foreground hover:text-foreground"><ChevronRight size={18} /></button>
              </div>
              <div className="flex items-center gap-2">
                <button type="button" aria-label="Previous year" onClick={() => setView((v) => ({ ...v, y: v.y - 1 }))} className="text-muted-foreground hover:text-foreground"><ChevronLeft size={16} /></button>
                <span className="text-body-sm font-semibold text-foreground">{view.y}</span>
                <button type="button" aria-label="Next year" onClick={() => setView((v) => ({ ...v, y: v.y + 1 }))} className="text-muted-foreground hover:text-foreground"><ChevronRight size={16} /></button>
              </div>
            </div>
