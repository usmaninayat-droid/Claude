  ...
  const base: FeedEntry[] = [
    ...
    ...list.slice(1, Math.max(1, idx + 1)).map((s, i): FeedEntry => {
      ...
      return {
        ...
        icon: <Pencil size={12} />,
        iconTone: tone,
        // Domain-agnostic phrasing — the stage chip carries the specifics (no hardcoded noun).
        text: 'moved to',
    ...
    <div className="flex flex-col gap-4">
      {rows.map((r) => (
        <div key={r.col} className="flex flex-col gap-1.5">
          <div className="text-caption font-semibold uppercase tracking-wide text-muted-foreground">{r.label}</div>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        <label className="text-caption font-semibold uppercase tracking-wide text-muted-foreground">Notes</label>
        <textarea
          value={note}
...
export function resolveRightPanelTab(
  ...
  // Resolve from a name fragment — `activ` wins over `timeline` so a tab named
  // ActivityFeed never resolves to the stage tracker.
  const pick = (s: string): React.ReactNode | null => {
    if (s.includes('activ')) return <ActivityPanel {...ctx} />;
    if (s.includes('timeline')) return <PipelineTimelinePanel {...ctx} />;
    if (s.includes('link')) return <LinkedItemsPanel {...ctx} />;
    if (s.includes('attach') || s.includes('note') || s.includes('file')) return <AttachmentsPanel {...ctx} />;
    return null;
  };
  const comp = (tab.component ?? '').toLowerCase();
  const key = (tab.key ?? '').toLowerCase();
  // The COMPONENT name wins over the key (so component:"ActivityFeed" key:"timeline"
  // renders the activity feed, not the stage tracker).
  const render = () => pick(comp) ?? pick(key) ?? <PanelEmpty>No panel renderer for “{tab.title}”.</PanelEmpty>;
