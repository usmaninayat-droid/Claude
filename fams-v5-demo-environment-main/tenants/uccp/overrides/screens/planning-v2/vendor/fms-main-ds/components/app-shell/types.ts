...
export interface ModuleConfig<TData = unknown> {
  ...
  /** When true, the shell suppresses its default ModuleToolbar row (search +
   *  filters + export) for this module. Modules whose views carry their own
   *  bespoke toolbars (Incidents Kanban + Hybrid) opt out here to avoid a
   *  duplicate search bar at the top of the screen. */
  hideToolbar?: boolean;
}
...
export interface MonitoringEntity {
  ...
  mapLabel?: string;
  /**
   * Fully custom map-pin artwork (a data URI or hosted image URL) — threaded
   * straight to the map layer's `MapMarker.iconUrl` (an `<img>`-based pin,
   * replacing the default teardrop/AssetMarker pin entirely). Use this when a
   * product needs a bespoke pin graphic (e.g. a Figma-exact composite
   * illustration) that the built-in pin kinds can't express. Omit to keep
   * the default pin pipeline (`kind`/`assetType`-driven) unchanged.
   */
  iconUrl?: string;
  /** Pixel size `[w, h]` for `iconUrl` artwork — defaults to the generic
   *  34×40 pin size (see `MapMarker.iconSize`); set this when the custom
   *  artwork has different native proportions (e.g. a wide composite
   *  plate+fill pin) so it isn't squashed into the default box. */
  ...
  /** Tanker/tank fill level 0–100. When any entity sets this, the fleet list
   *  gains a dedicated "Fill Level" column (a threshold-coloured bar), shown
   *  BEFORE the metric/Speed column. Purely additive — products that never
   *  set it render byte-identically (no extra column). */
  fillLevel?: number;
  /** Popup-header meta chips under the title (e.g. plate · driver · location),
   *  each an optional icon + text. Renders the reference vehicle-popup's
   *  identity row; omit to keep the plain title/subtitle header. */
  headerMeta?: { icon?: React.ReactNode; text: React.ReactNode }[];
  ...
  /** Telemetry grid in the on-map popup (Overview). `icon`/`tone` are
   *  additive/optional (default: no icon, default foreground colour) — a
   *  product can render an icon+label+value triplet colored by a semantic
   *  threshold (e.g. a fill-level %) without any visual change for existing
   *  consumers that only set `label`/`value`. */
  telemetry?: { label: React.ReactNode; value: React.ReactNode; icon?: React.ReactNode; tone?: 'success' | 'warning' | 'error' }[];
  ...
  /** Optional device rows in the popup (Devices tab). `imei`/`dataRec`/
   *  `value`/`tone` are additive/optional — when a row sets `imei`, the
   *  popup renders the fuller DEVICE NAME / IMEI / DATA REC / VALUE table
   *  (value colored by `tone`); rows that only set `name`/`meta` keep the
   *  original simple list row, so no existing consumer's look changes. */
  devices?: {
    id: string;
    name: React.ReactNode;
    meta?: React.ReactNode;
    imei?: React.ReactNode;
    dataRec?: React.ReactNode;
...
export interface MonitoringModuleData {
  ...
  listColumnLabels?: { entity?: string; overview?: string; metric?: string; fill?: string; status?: string; assignment?: string; activity?: string; speed?: string; timestamp?: string; fuelType?: string; vehicleId?: string; assetType?: string };
  /** When set, the fleet list becomes horizontally scrollable with this
   *  minimum inner width (px) — for column sets (e.g. Vehicle · Activity ·
   *  Fill · Speed) that don't fit the panel width. Omit to keep the list
   *  flush to the panel (existing behaviour, no horizontal scroll). */
  listMinWidth?: number;
  /** Opt-in: suppress the fleet-row's driver/subtitle second line, so the
   *  Vehicle cell renders as a single line (icon + ID only) — matches the
   *  Flood Management reference table (275-97176), whose rows never carry
   *  a second line. Omit to keep the existing driver/subtitle line other
   *  products may already rely on. */
  hideRowSubtitle?: boolean;
  /** Opt-in: suppress the "Showing N {listTitle}" count row above the
   *  table header (still shows the "Hide/Show applied filters" link when
   *  facet filters are active). The Flood Management reference table
   *  (275-97176) goes straight from the search row into the column
   *  header, with no count line between them. */
  ...
  /** Weather forecast overlay (bottom-center) — the Windy-style model-tab
   *  widget (`WeatherForecastWidget`). Optional + additive; omit → no change. */
  showWeather?: boolean;
  /** Simulated storm cells for the Map Layers "Rain (simulated)" weather
   *  toggle — drawn as stroke+20%-fill circles colour-coded by intensity,
   *  each with a centre chip showing rain level and flood-risk %. Omit →
   *  the toggle is hidden. */
  rainCells?: { id: string; center: LatLng; radiusM: number; rainMmHr: number; floodRiskPct: number }[];
  /** Demo weather stations for the "Stations — by temp" weather toggle:
   *  temp-coloured dot markers with a °C reading. Omit → toggle hidden. */
  weatherStations?: { id: string; position: LatLng; tempC: number }[];
  /** Demo cloud masses for the "Clouds" weather toggle: soft grey areas whose
   *  fill opacity tracks density (0–1). Omit → toggle hidden. */
  cloudMasses?: { id: string; center: LatLng; radiusM: number; density: number }[];
  /** Demo precipitation accumulation patches (mm, last hour) for the
   *  "Precipitation" weather toggle: blue-scale shaded areas. Omit → hidden. */
  precipAreas?: { id: string; center: LatLng; radiusM: number; mm: number }[];
  /** Analytics stack (Flood Index / Rain Projection / Fleet Status) shown
   *  top-left of the map. Omit → hidden. Purely presentational; the analytics
   *  data lives in the config so per-tenant tweaks stay declarative. */
  analytics?: import('../map').MapAnalyticsPanelProps;
  /** Layer toggles for the map's top-right layers control (Assembly Points,
   *  Vehicles, Black Spots, Nearby Incidents, Nearby Inspectors, …). */
  mapLayers?: import('../map').MapLayerToggle[];
  /** Auxiliary map layers — the 6-layer stack shared with the Incidents
   *  Hybrid view (POI · Assembly Points · Vehicles · Black Spots · Nearby
   *  Inspectors · Zones / Geofences). Each definition contributes markers
   *  and/or zones when its toggle is enabled, and gets its own right-column
   *  tip button + Layers-popover row. Nearby Incidents lives on the
   *  `incidents` field below because it has its OWN top-bar severity
   *  filter (Critical / Medium / Minor with counts), not a single toggle.
   *  Structural type (matches `AuxMapLayerDef` in
   *  `src/modules/aux-map-layers.tsx`) so the DS layer stays product-agnostic. */
  auxLayers?: {
    id: string;
    label: string;
    ...
    buildMarkers?: () => MapMarker[];
    buildZones?: () => import('../map').MapZone[];
  }[];
  /** Nearby Incidents severity filter — a distinct row of colored checkboxes
   *  in the map's top control bar (Critical / Medium / Minor with per-bucket
   *  counts + a dot in the severity token). Toggling a bucket adds/removes
   *  its matching incident markers from the map. */
  incidents?: {
    severities: { id: string; label: string; color: string; count: number }[];
    /** Builder receives the currently-checked severity ids. */
    buildMarkers: (enabledSeverities: Set<string>) => MapMarker[];
  };
  /** Location search — powers the top-bar search icon (fly-to on submit).
   *  Each entry surfaces in the search popover; picking one flies the map
   *  to that position. Falls back to the fleet-list search when omitted. */
  locations?: { id: string; label: string; sublabel?: string; position: LatLng; zoom?: number }[];
