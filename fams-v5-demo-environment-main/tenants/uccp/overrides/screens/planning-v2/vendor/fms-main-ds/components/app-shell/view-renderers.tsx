...
import { CheckCircle, Inbox01, Bell01, Activity, Clock, SearchMd, FilterFunnel02, Flag06, LayersThree01, BarChartSquare02, AlertTriangle } from '../../icons';
...
export function PipelineListView({
  ...
  data: PipelineModuleData;
  onOpenDetail?: (d: DetailDescriptor) => void;
  /** Flush table treatment (C&C): drop the outer padding and the table's
   *  own card border/background so it sits directly on the panel surface. */
  flush?: boolean;
  /**
   ...
   *
   * A module that hides the shell's default toolbar (`hideToolbar: true`,
   * e.g. Incidents — every tab carries its own inline toolbar) owns this
   * value itself instead and just passes it straight through here.
   */
  groupByCol?: string;
  /**
   * Row-selection checkbox column — off by default (unchanged behavior for
   * existing pipeline modules). A module opts in (e.g. Incidents' IIMS List
   * View Figma, which shows a leading checkbox per row) by passing all
   * three of these straight through to the underlying `DataTable`.
   */
  selectable?: boolean;
  selectedIds?: string[];
  ...
  const groupBy = groupColumn
    ? {
        ...
        // Map a status/stage key to a colored pill (Figma IIMS List View,
        // node 300-29545 — each group header is a stage-colored pill, not
        // plain text) when the key matches a known stage; otherwise show
        // the raw value as plain text (e.g. grouping by Priority/Assignee).
        label: (k: string) => {
          const stage = data.stages.find((s) => s.id === k);
          if (!stage) return k;
          return (
            <span
              className="inline-flex items-center whitespace-nowrap rounded-md px-2 py-0.5 text-caption font-semibold text-white"
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className={cn('min-h-0 flex-1', flush ? 'p-0' : 'p-4', data.listScrollMode === 'lazy' && !groupBy ? 'flex flex-col' : 'overflow-auto')}>
        <DataTable
          className={cn(data.listScrollMode === 'lazy' && !groupBy && 'h-full', flush && 'rounded-none border-0')}
          columns={cols}
          data={cards}
          emptyState={<EmptyState query={query} />}
