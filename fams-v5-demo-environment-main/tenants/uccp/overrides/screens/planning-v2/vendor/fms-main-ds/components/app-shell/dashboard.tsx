...
export interface DashboardProps {
  ...
  kpis?: DashboardKpi[];
  /** Fixed columns for the KPI grid on `lg` breakpoints (e.g. `5` for a
   *  5-per-row row). Omit to keep the default auto-fit (min 200px per tile). */
  kpiColumns?: number;
...
export function Dashboard({
  ...
  return (
    <div className={cn('flex flex-col gap-4 p-6', className)}>
      ...
      {/* KPI row — fixed columns when `kpiColumns` is set (e.g. 5-per-row);
          otherwise the default auto-fit (min 200px per tile). */}
      {kpis?.length ? (
        <div
          className={cn(
            'grid gap-3',
            kpiColumns ? 'grid-cols-2 md:grid-cols-3' : 'grid-cols-[repeat(auto-fit,minmax(200px,1fr))]',
          )}
          style={kpiColumns ? { gridTemplateColumns: `repeat(${kpiColumns}, minmax(0, 1fr))` } : undefined}
        >
          {kpis.map((k, i) => {
            const { id, ...rest } = k;
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          {field.label}
          {selected.length ? (
            <span className="inline-flex size-4 items-center justify-center rounded-full bg-primary text-caption font-bold text-primary-foreground">{selected.length}</span>
          ) : null}
        ...
        {field.options.map((o) => {
          ...
          return (
            <button key={o.value} type="button" onClick={() => toggle(o.value)} className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-body-sm hover:bg-muted">
              ...
              {o.count !== undefined ? <span className="shrink-0 rounded-full bg-muted px-1.5 py-0.5 text-caption font-semibold tabular-nums text-muted-foreground">{o.count}</span> : null}
            </button>
          );
        })}
      </PopoverContent>
