...
// 2026-08-31: `night` used to read `var(--primary)` — a status/identity glyph,
// not brand chrome, so it now reads the theme's tenant-stable `--status-info`
// role token instead (same role-separation fix as `plan-monitoring.tsx`'s
// `shiftGlyph`; keeps the same blue it always rendered, independent of a
// tenant's `--primary`).
const SHIFT: Record<PlanShift, { icon: React.ReactNode; color: string }> = {
  ...
  night: { icon: <Icons.Moon01 size={13} />, color: 'var(--status-info)' },
};
