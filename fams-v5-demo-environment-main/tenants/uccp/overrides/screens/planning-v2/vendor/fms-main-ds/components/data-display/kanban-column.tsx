...
export interface KanbanColumnProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  counterText?: string;
  /**
   * Board variant only — whether the header shows the 4px colored
   * top-border accent (Truemax fleet/maintenance Figma). Some product
   * Figmas (e.g. IIMS Incidents) tint the whole column body instead and
   * keep the header itself plain white with no accent border — set this
   * to `false` to match that look. Defaults to `true` (unchanged for
   * existing "board" consumers).
   */
  topBorder?: boolean;
  ...
  (
    {
      ...
      width = '280px', cardIds, variant = 'plain', tint, counterBg, counterText, topBorder = true, children, ...props
    },
    ref
  ) => {
        ...
        <SortableContext items={resolvedCardIds} strategy={verticalListSortingStrategy}>
          <div
            className={cn(
              ...
              board ? 'w-full space-y-4' : 'space-y-2 p-2',
            )}
            /* Hide the scrollbar visually so the cards use the full column
             * width — otherwise the browser reserves 11–15px of scrollbar
             * gutter inside the body and the card row ends short of the
             * header strip above it. Wheel scroll still works. */
