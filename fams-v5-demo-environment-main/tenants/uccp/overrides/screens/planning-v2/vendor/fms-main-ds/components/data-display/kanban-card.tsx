...
export interface KanbanCardProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'title' | 'id'> {
  ...
  /** Optional icon that replaces the default Calendar next to `dateLabel`.
   *  Accepts a lucide-react icon name (e.g. "Building2") or a React node.
   *  Consumers use this when the footer-right slot represents something other
   *  than a date — e.g. an Onwani number rendered with a Building icon. */
  dateIcon?: string | React.ReactNode;
          ...
          <div className="relative flex w-full items-center justify-between pt-4">
            ...
            {dateLabel ? (
              <div className="inline-flex items-center gap-1">
                {dateIcon !== undefined
                  ? resolveIcon(dateIcon, 14, isOverdue ? 'var(--destructive)' : 'var(--muted-foreground)')
                  : (
                    <Calendar
                      size={14}
