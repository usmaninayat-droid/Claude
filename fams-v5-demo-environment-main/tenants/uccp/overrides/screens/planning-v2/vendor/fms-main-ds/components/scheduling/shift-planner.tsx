...
function WorkerAvatar({ name, color = 'var(--primary)', size = 32 }: { name: string; color?: string; size?: number }) {
  ...
  return (
    <span
      ...
      style={{ width: size, height: size, background: color, fontSize: Math.max(12, size * 0.36) }}
    >
      {initials || '—'}
    </span>
  );
...
function ScheduleShiftSheet({
  ...
  /* ── create-mode stepper (Figma 512-44117 "Schedule New Shift": Basic Info →
   *    Scheduled Plans → Shift Info, advanced with a full-width "Save & Next";
   *    the final step submits with "Create"). Edit mode keeps the single-page
   *    grouped layout — the wizard only makes sense when building from scratch. */
  const steps = React.useMemo(() => (
    mode === 'create'
      ? [
          { id: 'basic', label: 'Basic Info' },
          ...(hasPlans ? [{ id: 'plans', label: 'Scheduled Plans' }] : []),
  ...
  const stepId = stepped ? steps[stepIdx].id : null;
      ...
      <SheetContent side="right" width="min(520px, 94vw)" className="p-0">
            ...
            {steps.map((s, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'relative py-2.5 text-body-sm font-semibold transition-colors',
                    active ? 'text-primary' : reachable ? 'text-foreground hover:text-primary' : 'text-muted-foreground',
                  )}
                >
                  {s.label}
        ...
        <SheetFooter>
          {mode === 'edit'
            ...
            : stepped && !lastStep
              ? <Button variant="primary" className="w-full" onClick={() => setStepIdx((i) => i + 1)}>Save &amp; Next</Button>
