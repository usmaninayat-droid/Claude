  ...
  (
    ...
    const header = board ? (
      ...
      <div className="relative h-[42px] w-full shrink-0">
        <div className="flex size-full items-center justify-between rounded-[6px] border border-border bg-card px-2 pb-2.5 pt-3">
          <div className="flex items-center gap-1">
            <span className="whitespace-nowrap text-[14px] font-semibold text-foreground">{label}</span>
            {typeof count === 'number' ? (
              <span
                className="inline-flex min-w-[22px] items-center justify-center rounded-[18px] px-1 py-1.5 text-[10px] font-extrabold leading-[10px]"
          ...
          {aggregateMetric ? (
            <span className="text-[11px] tabular-nums text-muted-foreground" title={aggregateMetric.tooltip}>
      ...
      <div className="flex items-center gap-2 border-b border-border px-3 py-2.5">
        ...
        {aggregateMetric ? (
          <span className="ml-2 text-[11px] tabular-nums text-muted-foreground" title={aggregateMetric.tooltip}>
            {aggregateMetric.value}
          </span>
        ) : null}
