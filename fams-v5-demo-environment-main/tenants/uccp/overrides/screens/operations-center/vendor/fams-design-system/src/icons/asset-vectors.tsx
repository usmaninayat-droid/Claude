...
// PROVENANCE (2026-09-01, GIS Inspectors-tab marker fix): the workforce art
// file is `Default Workforce.svg`, which slugifies to `default-workforce` —
// so callers passing the generic kind name `"workforce"` (as
// `LiveGisMap.tsx`'s inspector markers do) got NO match and `AssetGlyph`
// silently rendered nothing, which is exactly why those map pins showed as
// empty teardrop circles. Alias the bare kind name to its single workforce
// entry so `name="workforce"` resolves the same real art `name="Default
// Workforce"` does, without renaming the source file or every call site.
if (!REGISTRY.workforce) {
  const workforceEntry = Object.values(REGISTRY).find((e) => e.kind === 'workforce');
  if (workforceEntry) REGISTRY.workforce = workforceEntry;
}
