...
import { StatePill, StateTransitionToolbar, PeoplePicker } from '../data-display';
...
/**
 * Detail-header status control: the current StatePill + transition buttons. A
...
function StatusTransitionControl({
  ...
  return (
    <span className="flex items-center gap-2">
      <StatePill label={stage.label} bg={stage.color} size="sm" />
      {transitions.length ? (
...
function pipelineDetail(
  ...
  const ctx: CellRenderContext = { config, refLabel: opts.refLabel };
  ...
  return {
    ...
    render: (actions: ShellActions) => {
      ...
      const detailsLeft = dm.details.filter((c) => c.pos !== 'right').map(renderField);
      const detailsRight = dm.details.filter((c) => c.pos === 'right').map(renderField);
