      ...
      <div className="flex w-full flex-col gap-6">
        {sections.map((section, i) => (
          <div key={section.label ?? i} className="flex w-full flex-col">
            {section.label ? (
              <div className="py-2 text-[12px] font-semibold text-muted-foreground">{section.label}</div>
            ) : null}
            {section.items.map((item) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex w-full items-center gap-3 rounded-[4px] px-3 py-2.5 text-left text-[14px] outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring',
                    item.active
