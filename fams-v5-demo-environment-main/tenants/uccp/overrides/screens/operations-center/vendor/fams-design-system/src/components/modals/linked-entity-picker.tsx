    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="max-w-[520px]">
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="flex h-full min-h-[160px] items-center justify-center px-6 py-12 text-center text-sm text-muted-foreground">
...
export function LinkedEntityTile({ label, avatarSrc, onRemove, className }: LinkedEntityTileProps) {
  return (
    <span
      className={cn(
        'inline-flex items-center gap-2 rounded-full border border-border bg-card py-1 pl-1 pr-2 text-xs text-foreground shadow-sm',
        className
      )}
    >
