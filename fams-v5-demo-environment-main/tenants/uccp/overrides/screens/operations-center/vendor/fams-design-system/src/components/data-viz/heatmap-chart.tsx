  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      ...
      <div className="relative overflow-x-auto">
          ...
          <tbody>
            {yCategories.map((y) => (
              <tr key={y}>
                <th
                  scope="row"
                  className="pr-3 text-right text-[11px] font-medium text-muted-foreground"
        ...
        {hover ? (
          <div className="pointer-events-none absolute right-0 top-0 rounded-md bg-popover px-3 py-2 text-xs text-popover-foreground shadow-lg">
      ...
      {xAxisLabel ? (
        <p className="text-center text-[11px] text-muted-foreground">{xAxisLabel}</p>
      ) : null}
    </div>
  );
}
