...
export function TopNavModule({
    ...
    return (
      ...
      >
        <div className="flex h-full shrink-0 items-center gap-2.5 px-6">
          ...
          <span className="text-[16px] font-semibold text-foreground">{moduleName}</span>
        </div>

        {/* Segmented view-tab strip — each tab is bordered (l/r), overlapping. */}
        <div className="flex h-full items-center">
          {views.map((v) => {
            const Icon = VIEW_ICON[v.kind] ?? LayoutGrid;
            return (
              <div
                key={v.id}
                role="tab"
              ...
              >
                ...
                <span
                  className="max-w-[160px] truncate text-[12px] font-semibold"
        ...
        {rightSlot ? <div className="ml-auto flex items-center gap-2 px-4">{rightSlot}</div> : null}
      </header>
    );
  }
  ...
  return (
    <header className={cn('flex h-12 shrink-0 items-center gap-3 border-b border-border bg-background px-4', className)}>
      <div className="shrink-0 text-sm font-semibold text-foreground">{moduleName}</div>
    ...
    >
      {showTrafficLights ? (
        <div className="flex items-center gap-1.5 pl-1 pr-3" aria-hidden>
          <span className="block h-3 w-3 rounded-full" style={{ background: '#F04438' }} />
          <span className="block h-3 w-3 rounded-full" style={{ background: '#F79009' }} />
        </div>
      ) : null}
      ...
      {onBack ? (
        <button
          type="button"
          onClick={onBack}
          className="mr-1 rounded-md px-2 py-1 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
        >
          ← Back
        </button>
      ) : null}
    ...
    >
      ...
      >
        ...
        <span className="flex min-w-0 flex-col items-start leading-tight">
          ...
          <span className={cn('truncate text-xs', tab.active ? 'font-semibold text-foreground' : 'text-foreground/80')}>
            {tab.label}
          </span>
        </span>
      </button>
