...
export function AppShell({
  ...
  // Monotonic counter for dynamically-added view tabs — guarantees unique tab
  // ids (and therefore independent per-tab toolbar/filter state) even when
  // multiple tabs of the same view kind are open.
  ...
  // Always opens a NEW tab (multiple tabs of the same view kind are allowed);
  // duplicates get a numbered label. Each new tab has a unique id → its own
  // independent toolbar/filter/sort state (keyed by module::tab).
  const addView = (kind: ViewKind) => {
    ...
    const tab: ModuleTab = { id: `${kind}-${(viewSeq.current += 1)}`, kind, label };
    setViews([...tabs, tab]);
  ...
  // The picker offers EVERY view kind the module supports (not just the ones not
  // yet open) — a user may open a second Kanban/List/Hybrid tab with its own
  // filters. Each pick creates a fresh independent tab.
