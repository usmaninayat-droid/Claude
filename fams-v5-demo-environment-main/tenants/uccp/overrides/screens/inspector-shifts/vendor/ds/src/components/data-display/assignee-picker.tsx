  ...
  if (current && disabled) {
    return (
      <span className={cn('flex items-center gap-2', className)}>
        ...
        <span className="text-[14px] font-semibold text-foreground">{current.name}</span>
      </span>
    );
  }
      ...
      <PopoverTrigger asChild disabled={disabled}>
        {current ? (
          ...
          <button
            type="button"
            className={cn(
              'flex items-center gap-1.5 rounded-full border border-dashed border-primary/50 py-1 pl-1.5 pr-2.5 text-[12px] font-semibold text-primary outline-none transition-colors hover:bg-secondary focus-visible:ring-2 focus-visible:ring-ring',
              className
            )}
            aria-label="Assign"
          >
