...
import { Hash, Check, ArrowRight, Home, FileText, X, Plus, Minus, Bookmark, MoreHorizontal, Trash2, ChevronLeft, ChevronRight, ChevronDown, Eye, EyeOff, MapPin, Layers, Maximize2, Flag, Tag, Calendar, Building2, Container } from 'lucide-react';
...
import type { LeafletMapHandle } from '../map';
...
/** Hex (#rrggbb) → rgba string at the given alpha. Falls back to the input. */
...
export function KanbanView({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <KanbanBoard
        className="flex min-h-0 flex-1 gap-2 overflow-x-auto p-4"
        onCardMove={move}
...
export function PipelineListView({
  ...
  const [groupCol, setGroupCol] = React.useState<string | null>(null);
  if (!data.columns) {
    return (
  ...
  const cards = filterPipelineCards(data, query, filters, facetFilters, sort);
  const cols = data.columns;
  const groupColumn = groupCol ? cols.find((c) => c.id === groupCol) : undefined;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-2 px-4 pt-3">
        <label className="text-body-sm text-muted-foreground" htmlFor="pipeline-group-by">Group by</label>
        <select
  ...
  return (
    <div className="grid h-full grid-cols-1 gap-4 p-4 lg:grid-cols-2">
      ...
      <div className="min-h-0">
        {data.map ? (
          <MapWidget height="100%" showZoomControls>
...
/* ── pipeline hybrid (cards + map) ───────────────────────────────────────
   Modeled on the Live Monitoring hybrid surface. LEFT: the pipeline's cards
   (same chrome as the kanban board) stacked in a collapsible rail — each with an
   eye toggle + a selected highlight; clicking one flies the map to its pin and
   opens its detail. RIGHT: a full-bleed map with a layers toggle that switches
   between PINS (priority-coloured dots + a checkbox legend) and ZONES (the
   service-area polygons), plus fit / zoom controls. */

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const HYBRID_META_ICON: Record<string, React.ComponentType<any>> = {
  MapPin, Tag, Calendar, Building2, Clock, Container, Hash,
};

/** A pipeline card for the hybrid left rail — mirrors the kanban card chrome
 *  (id + stage pill + priority flag, title, metadata, avatar/date footer) and
 *  adds an eye toggle (show/hide on map) + a selected ring. */
function HybridCard({ card, stage, selected, hiddenOnMap, dimmed, onSelect, onToggleMap }: {
  ...
  const pc = card.priorityConfig;
  const stageColor = stage?.color ?? 'var(--muted-foreground)';
  return (
    <div
      onClick={onSelect}
      className={cn(
        'group relative cursor-pointer rounded-[8px] border bg-card p-3 transition-shadow hover:shadow-sm',
        selected ? 'border-primary ring-1 ring-primary' : 'border-border',
        (hiddenOnMap || dimmed) && 'opacity-60',
      )}
    >
      <div className="flex items-start justify-between gap-1.5">
        <div className="flex min-w-0 flex-wrap items-center gap-1.5">
          <button
            ...
            aria-label={hiddenOnMap ? 'Show on map' : 'Hide from map'}
            aria-pressed={!hiddenOnMap}
            className={cn('grid size-5 shrink-0 place-items-center rounded transition-colors',
              hiddenOnMap ? 'text-muted-foreground hover:bg-muted' : 'bg-secondary text-primary hover:bg-secondary/70')}
          >
            {hiddenOnMap ? <EyeOff size={13} /> : <Eye size={13} />}
          </button>
          {card.ticketId ? (
            <span className="inline-flex h-5 items-center gap-1 rounded-[2px] bg-border px-1.5">
              <Hash size={10} className="text-muted-foreground" />
              <span className="text-[11px] font-semibold uppercase leading-none text-muted-foreground">{card.ticketId}</span>
            </span>
          ) : null}
          {stage ? (
            <span className="inline-flex h-5 items-center rounded-[2px] px-1.5 text-[10px] font-semibold uppercase leading-none"
        ...
        {card.priority && pc ? (
          <span className="inline-flex h-5 shrink-0 items-center gap-1 rounded-[2px] px-1.5" style={{ background: pc.bg }}>
            <Flag size={10} style={{ color: pc.flagStroke, fill: pc.flagFill }} strokeWidth={2.2} />
            <span className="text-[10px] font-semibold uppercase leading-none" style={{ color: pc.text }}>{card.priority}</span>
          </span>
        ) : null}
      </div>
      {card.title ? <p className="mt-2 text-body-sm font-bold leading-snug text-foreground [overflow-wrap:anywhere]">{card.title}</p> : null}
      {card.metadataFields?.length ? (
        <div className="mt-1.5 flex flex-col gap-1">
          {card.metadataFields.map((f, i) => {
            const Ic = typeof f.icon === 'string' ? HYBRID_META_ICON[f.icon] : undefined;
            return (
              <span key={i} className="inline-flex items-center gap-1.5 text-caption text-muted-foreground">
                {Ic ? <Ic size={13} /> : typeof f.icon !== 'string' ? f.icon : null}
      ...
      {(card.avatars?.length || card.dateLabel) ? (
        <div className="mt-3 flex items-center justify-between border-t border-border pt-2.5">
          <span className="flex -space-x-1.5">
            {(card.avatars ?? []).map((a, i) => (
              <span key={i} className="grid size-6 place-items-center rounded-full border-2 border-card text-[10px] font-semibold text-white" style={{ background: a.color }}>{a.letter}</span>
            ))}
          </span>
          {card.dateLabel ? (
            <span className={cn('inline-flex items-center gap-1 text-caption font-medium', card.isOverdue ? 'text-[var(--status-error)]' : 'text-muted-foreground')}>
              <Calendar size={12} />{card.dateLabel}
            </span>
          ) : null}
        </div>
      ) : null}
...
/** Square map-tool button (matches the Live Monitoring toolbar). */
function HybridMapBtn({ label, active, onClick, children }: { label: string; active?: boolean; onClick?: () => void; children: React.ReactNode }) {
  return (
    <button type="button" aria-label={label} onClick={onClick}
      className={cn('grid size-8 place-items-center rounded-md border shadow-sm transition-colors',
...
export function PipelineHybridView({
  ...
  const [mode, setMode] = React.useState<'pins' | 'zones'>('pins');
  const [collapsed, setCollapsed] = React.useState(false);
  const toggle = (id: string) =>
    setHidden((prev) => { const n = new Set(prev); if (n.has(id)) n.delete(id); else n.add(id); return n; });

  // Priority legend — derived from any cards that carry a priority + colour.
  const priColor = (c: PipelineCardModel) => c.priorityConfig?.flagFill ?? c.priorityConfig?.text ?? undefined;
  const legend = React.useMemo(() => {
    const m = new Map<string, string>();
    for (const c of geoCards) {
      const l = typeof c.priority === 'string' ? c.priority : undefined;
      const col = priColor(c);
      if (l && col && !m.has(l)) m.set(l, col);
    }
    return Array.from(m, ([label, color]) => ({ label, color }));
  }, [geoCards]);
  const [offPri, setOffPri] = React.useState<Set<string>>(() => new Set());
  const togglePri = (l: string) =>
    setOffPri((prev) => { const n = new Set(prev); if (n.has(l)) n.delete(l); else n.add(l); return n; });
  const priActive = (c: PipelineCardModel) => !(typeof c.priority === 'string' && offPri.has(c.priority));

  const stageOf = (id?: string) => data.stages.find((s) => s.id === id);
  const label = (c: PipelineCardModel) => String(c.title ?? c.ticketId ?? '');

  const visible = geoCards.filter((c) => !hidden.has(c.id) && priActive(c));
  const markers = mode === 'pins'
    ? visible
        .filter((c) => c.location)
        .map((c) => ({ id: c.id, position: c.location!, kind: 'dot' as const, status: 'default' as const, color: priColor(c), selected: c.id === selectedId, tooltip: label(c) }))
    : [];
  const zones = mode === 'zones' ? (data.mapZones ?? []) : [];
  const center = data.mapCenter ?? geoCards.find((c) => c.location)?.location ?? data.mapZones?.[0]?.points[0] ?? [25.2, 55.27];

  const select = React.useCallback((c: PipelineCardModel) => {
    setSelectedId(c.id);
    if (c.location) mapRef.current?.flyTo(c.location, 14);
    if (data.toDetail && onOpenDetail) onOpenDetail(data.toDetail(c));
  }, [data, onOpenDetail]);
  ...
  return (
    <div className="flex h-full min-h-0 items-stretch">
      {/* LEFT rail — pipeline cards (collapsible) */}
      <aside className={cn('flex flex-col overflow-hidden border-r border-border bg-muted/20 transition-[width] duration-300', collapsed ? 'w-0 shrink-0' : 'w-[344px] shrink-0')}>
        <div className="flex shrink-0 items-center justify-between px-4 py-2.5 text-caption text-muted-foreground">
          <span>Showing <span className="font-semibold text-foreground">{visible.length}</span> of {geoCards.length}</span>
          {mode === 'zones' ? <span className="inline-flex items-center gap-1"><Layers size={12} /> Zones</span> : null}
        </div>
        <div className="flex min-h-0 flex-1 flex-col gap-2.5 overflow-auto px-4 pb-4">
          {geoCards.map((c) => (
            <HybridCard
              key={c.id}
              card={c}
              stage={stageOf(c.stageId)}
              selected={c.id === selectedId}
              hiddenOnMap={hidden.has(c.id)}
              dimmed={!priActive(c)}
              onSelect={() => select(c)}
              onToggleMap={() => toggle(c.id)}
            />
          ))}
        </div>
      </aside>

      {/* RIGHT — map. `isolate` gives the map its own stacking context so
          Leaflet's high internal z-indexes (panes/controls up to 1000) stay
          contained and don't paint over the detail side-sheet (z-50). */}
      <div className="relative isolate min-w-0 flex-1">
        <LeafletMap
          ...
          onMarkerClick={(id) => { const c = visible.find((x) => x.id === id); if (c) select(c); }}
        />

        {/* list collapse handle at the seam */}
        <button
          type="button"
        ...
        {/* top-right toolbar: layers toggle (pins ↔ zones) + fit */}
        <div className="absolute right-3 top-3 z-[1000] flex flex-col gap-1.5">
          {data.mapZones?.length ? (
            <HybridMapBtn label={mode === 'zones' ? 'Show pins' : 'Show zones'} active={mode === 'zones'} onClick={() => setMode((m) => (m === 'zones' ? 'pins' : 'zones'))}>
              {mode === 'zones' ? <MapPin size={15} /> : <Layers size={15} />}
            </HybridMapBtn>
          ) : null}
          <HybridMapBtn label="Fit to all" onClick={() => mapRef.current?.fitAll()}>
            <Maximize2 size={15} />
          </HybridMapBtn>
        </div>

        {/* priority legend (pins mode only) */}
        {mode === 'pins' && legend.length > 0 ? (
          <div className="absolute right-3 top-[92px] z-[900] flex flex-col gap-1.5 rounded-lg border border-border bg-card/95 p-2.5 shadow-md backdrop-blur">
            {legend.map(({ label: l, color }) => (
              <label key={l} className="flex cursor-pointer select-none items-center gap-2 text-caption font-medium text-foreground">
                <input type="checkbox" checked={!offPri.has(l)} onChange={() => togglePri(l)} className="size-3.5 accent-[var(--primary)]" />
        ...
        {/* zoom controls */}
        <div className="absolute bottom-4 right-4 z-[1000] flex flex-col overflow-hidden rounded-md border border-border bg-card shadow-sm">
          <button type="button" aria-label="Zoom in" onClick={() => mapRef.current?.zoomIn()} className="grid size-8 place-items-center text-foreground transition-colors hover:bg-muted"><Plus size={15} /></button>
          <div className="h-px bg-border" />
          <button type="button" aria-label="Zoom out" onClick={() => mapRef.current?.zoomOut()} className="grid size-8 place-items-center text-foreground transition-colors hover:bg-muted"><Minus size={15} /></button>
        </div>
      </div>
    </div>
  );
...
function InboxRow({ n, onClear }: { n: InboxNotification; onClear: (id: string) => void }) {
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          {n.dueLabel ? (
            <span className="inline-flex items-center gap-1 rounded-md bg-[var(--status-warning)]/10 px-1.5 py-0.5 text-[11px] font-semibold text-[var(--status-warning)]">
              <Clock size={11} />
              {n.dueLabel}
            </span>
          ) : null}
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        <div className="flex items-center gap-1.5">
          ...
          {n.timestamp ? <span className="text-[11px] tabular-nums text-muted-foreground">{n.timestamp}</span> : null}
        </div>
        <button
          type="button"
...
export function InboxView({ data, className }: { data: InboxModuleData; className?: string }) {
      ...
      <div className="flex items-center gap-6 border-b border-border px-7">
        {INBOX_TABS.map((t) => {
          ...
          return (
            ...
            >
              {t.label}
              <span className={cn('rounded px-1.5 py-0.5 text-[11px] font-semibold tabular-nums', active ? 'bg-secondary text-primary' : 'bg-muted text-muted-foreground')}>
            ...
            {order.map((g) => (
              <div key={g} className="mb-5">
                ...
                <div className="flex flex-col gap-2">
                  {byGroup.get(g)!.map((n) => (
                    <InboxRow key={n.id} n={n} onClear={clearOne} />
                  ))}
                </div>
              </div>
            ))}
    ...
    <div className="group relative flex h-full flex-col items-start gap-3 rounded-xl border border-border bg-card p-5 transition-all hover:border-primary hover:shadow-sm">
      ...
      <button type="button" onClick={onOpen} className="flex min-w-0 flex-1 flex-col items-start gap-1 text-left outline-none">
        <div className="flex items-center gap-2">
          ...
          {system ? (
            <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">System</span>
          ) : null}
        </div>
...
export function ReportsHome({
  ...
  const [active, setActive] = React.useState<string>(categories[0] ?? 'saved');
  const [query, setQuery] = React.useState('');
  ...
  const visibleSystem = systemReports.filter((r) => (r.category ?? 'Reports') === active && matches(r.label, r.description));
      ...
      <aside className="flex w-[220px] shrink-0 flex-col gap-1 border-r border-border bg-card p-3">
        {showSaved ? (
          <>
            ...
            <div className="px-3 pb-1 pt-3 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">System</div>
          </>
        ) : null}
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-auto px-7 py-5">
          <div className="mb-3 flex items-center gap-2">
            <span className="text-body font-semibold uppercase tracking-wide text-foreground">{active === 'saved' ? 'Saved' : active}</span>
            {active !== 'saved' ? (
              <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">System</span>
            ) : null}
          </div>
