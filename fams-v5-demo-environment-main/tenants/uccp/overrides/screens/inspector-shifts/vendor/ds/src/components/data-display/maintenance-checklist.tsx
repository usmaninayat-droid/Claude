...
const STATE_CONFIG: Record<ChecklistItemState, {
  ...
  passed:  { bg: 'rgba(18,183,106,0.1)', border: 'var(--success-500, #12B76A)', iconColor: 'var(--success-500, #12B76A)', label: 'Passed', Icon: Check },
  failed:  { bg: 'rgba(240,68,56,0.1)',  border: 'var(--destructive)', iconColor: 'var(--destructive)',     label: 'Failed', Icon: X },
  na:      { bg: 'rgba(102,112,133,0.1)',border: 'var(--muted-foreground)', iconColor: 'var(--muted-foreground)', label: 'N/A',   Icon: Minus },
};
...
function ChecklistRow({
  ...
  return (
    <li className="flex items-start gap-3 px-4 py-3">
      <span
        aria-hidden
        className="mt-0.5 inline-flex size-5 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold text-muted-foreground"
      ...
      {noteOpen && editable ? (
        <div className="mt-2 w-full basis-full">
          <input
            ...
            className="w-full rounded-md border border-border bg-input-background px-2 py-1 text-xs text-foreground outline-none focus:border-primary"
          />
        </div>
      ) : null}
    </li>
