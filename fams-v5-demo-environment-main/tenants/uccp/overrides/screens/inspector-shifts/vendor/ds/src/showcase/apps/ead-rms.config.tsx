  ...
  return {
    ...
    render: () => (
        <EntityDetail
          ...
          avatarColor="#004B87"
          statusOverlay={
            <span className="inline-flex items-center rounded-[2px] px-1.5 py-1 text-[10px] font-semibold uppercase tracking-[0.5px] text-white" style={{ background: statusBg }}>
              {v.status}
            </span>
          }
          name={v.plate}
                ...
                <div className="flex flex-col">
                  {manifestData.rows.filter((m) => m.vehicle === v.plate).map((m, i, arr) => (
                    <div key={m.id} className={i < arr.length - 1 ? 'border-b border-border py-2' : 'py-2'}>
                      ...
                      <p className="text-[11px] text-muted-foreground">{m.producer} → {m.facility}</p>
                    </div>
                  ))}
