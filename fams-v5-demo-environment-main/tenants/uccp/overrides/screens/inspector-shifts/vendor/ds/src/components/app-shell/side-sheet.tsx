...
import { Input, Textarea, Label, Button, Switch } from '../primitives';
...
export function FieldControl({
  ...
  if (f.type === 'picker') {
    return <EntityPickerControl field={f} value={value} onChange={onChange} />;
  }
          ...
          f.type === 'toggle' ? (
            ...
            <div key={f.key} className="col-span-2 flex items-center justify-between gap-4">
              <div className="min-w-0">
                <p className="text-[12px] font-semibold leading-[14px] text-muted-foreground">{f.label}</p>
                {f.placeholder ? (
