...
export interface EntityDetailProps {
  ...
  image?: string;
  /** How the hero image sits in its box. 'cover' (default) fills+crops — right
   *  for photos; 'contain' fits the whole graphic — right for illustrations/logos. */
  imageFit?: 'cover' | 'contain';
  /** Background behind a 'contain' hero image (defaults to the muted panel bg). */
  imageBg?: string;
    ...
    <div className={cn('flex h-full min-h-0 items-stretch', className)}>
      ...
      <aside className="w-[320px] shrink-0 overflow-y-auto border-r border-border bg-muted/40">
        <div className="flex flex-col gap-6 p-7">
          ...
          <div className="relative w-full">
            <div
              className="h-[220px] w-full overflow-hidden rounded-[4px] bg-muted"
              style={image && imageBg ? { background: imageBg } : undefined}
            >
              {image ? (
                <img
                  src={image}
                  alt=""
                  className={cn('h-full w-full', imageFit === 'contain' ? 'object-contain p-4' : 'object-cover')}
                  loading="lazy"
                />
              ) : (
                <div
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden bg-card">
        <div className="shrink-0 px-7 pt-5">
          <div className="flex items-center gap-8 overflow-x-auto border-b border-border [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="tablist">
            {tabs.map((t) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'shrink-0 whitespace-nowrap pb-2.5 text-[14px] outline-none transition-colors',
                    active
