...
const generatorData: EntityModuleData<Generator> = {
  ...
  toDetail: (row) => ({
    ...
    render: () => {
      ...
      const statusBadge = (
        <span className="inline-flex items-center rounded-[2px] px-1.5 py-1 text-[10px] font-semibold uppercase tracking-[0.5px] text-white" style={{ background: genStatusPill(live.status) }}>
          {live.status}
        </span>
      );
      return (
