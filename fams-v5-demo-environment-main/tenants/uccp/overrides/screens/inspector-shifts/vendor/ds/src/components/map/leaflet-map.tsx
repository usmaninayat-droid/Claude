...
export interface MapMarker {
  ...
  status?: MarkerStatus;
  /** Explicit marker colour — overrides the status-derived colour when set. */
  color?: string;
...
export interface LeafletMapProps {
  ...
  /** Tile layer. Defaults to OSM standard raster (key-free). */
  tileUrl?: string;
...
/**
 * TILE SOURCE (2026-09-01, Carto key regression — same fix as
 * operations-center/src/features/dashboard/LiveGisMap.tsx commit 2dbc789):
 * `{s}.basemaps.cartocdn.com` now requires a Carto API key for every raster
 * style. Tiles still return 200, but the PNG body is an "API KEY REQUIRED"
 * watermark card — which is exactly what the Schedule-New-Shift sector map
 * preview was rendering. Replaced with the key-free OSM standard raster used
...
function markerHtml(m: MapMarker): { html: string; size: [number, number]; anchor: [number, number] } {
  const color = m.color ?? STATUS_COLORS[m.status ?? 'default'];
  const label = m.label
