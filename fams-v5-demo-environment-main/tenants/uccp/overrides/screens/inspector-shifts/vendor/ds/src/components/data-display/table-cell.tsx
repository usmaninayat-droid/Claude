...
export function TableCell({ kind = 'default', value, className, align = 'left', onClick }: TableCellProps) {
  ...
  switch (kind) {
      ...
      return (
        <td className={cn('px-3 py-2', alignClass, className)}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs tabular-nums" style={{ color: colorCss }}>{pct}%</span>
          </div>
        </td>
      );
    }
      ...
      return (
        <td className={cn('px-3 py-2', alignClass, className)}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs tabular-nums text-muted-foreground">{Number(v?.percent ?? 0)}%</span>
          </div>
        </td>
      );
    ...
    case 'group-title':
      return (
        <td colSpan={v?.colSpan ?? 1} className={cn('bg-muted px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground', className)}>
          {v?.label}
        </td>
      );
        ...
        <td className={cn('px-3 py-2', alignClass, className)}>
          <div className="flex -space-x-2">
            ...
            {Array.isArray(v) && v.length > 3 ? (
              <span className="ml-1 inline-flex items-center justify-center rounded-full bg-muted px-1.5 text-xs text-muted-foreground">+{v.length - 3}</span>
            ) : null}
          </div>
        </td>
      );
    ...
    default:
      return (
        <td className={cn('px-3 py-2 text-sm text-foreground', alignClass, className)} onClick={onClick}>
