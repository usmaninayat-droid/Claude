...
export function CriticalEventsList({ events, emptyState, className, ...props }: CriticalEventsListProps) {
  if (events.length === 0) {
    return (
      <div className={cn('rounded-lg border border-border bg-card p-6 text-center text-sm text-muted-foreground', className)} {...props}>
      ...
      {events.map((e) => {
        ...
        return (
          ...
          >
            ...
            {e.timestamp ? <span className="shrink-0 text-[11px] tabular-nums text-muted-foreground">{e.timestamp}</span> : null}
          </button>
        );
      })}
    </div>
