  ...
  toDetail: (r) => ({
    ...
    render: (actions) => {
      ...
      const stagePill = (stageId?: string) => {
        ...
        return (
          <span className="inline-flex items-center rounded-[2px] px-1.5 py-1 text-caption font-semibold uppercase tracking-[0.5px] text-white" style={{ background: s?.color ?? '#667085' }}>
            {s?.label ?? '—'}
          </span>
        );
      };
      return (
        <EntityDetail
          ...
          avatarColor="#16a34a"
          statusOverlay={
            <span className="inline-flex items-center rounded-[2px] px-1.5 py-1 text-caption font-semibold uppercase tracking-[0.5px] text-white" style={{ background: companyStatusPill(r.status) }}>
              {r.status}
            </span>
          }
          name={r.name}
                    ...
                    <EntityChartCard title="Key Contacts" subtitle="People on this account">
                      {companyContacts.slice(0, 4).map((ct, i, arr) => (
                        <EntityListItem
                          key={ct.id}
                          leading={
                            <span className="grid size-6 place-items-center rounded-full bg-primary text-caption font-bold text-white">
                              {ct.name.split(' ').map((w) => w[0]).join('').slice(0, 2)}
                            </span>
                          }
                          title={ct.name}
                          trailing={<span className="text-caption font-medium text-muted-foreground">{ct.role}</span>}
                ...
                <div className="flex flex-col">
                  {companyDeals.map((c, i, arr) => (
                    <EntityListItem
                      key={c.id}
                      leading={
                        <span className="inline-flex items-center rounded-[3px] px-1.5 py-0.5 text-caption font-bold uppercase tracking-[0.3px]" style={{ background: c.typeConfig?.bg ?? '#f2f4f7', color: c.typeConfig?.text ?? '#667085' }}>
                          {String(c.type ?? '')}
                        </span>
                      }
                      title={String(c.title ?? '')}
    ...
    render: (actions) => {
      ...
      return (
        <EntityDetail
          ...
          avatarColor="#16a34a"
          statusOverlay={
            <span className="inline-flex items-center rounded-[2px] bg-black/40 px-1.5 py-1 text-caption font-semibold uppercase tracking-[0.5px] text-white">
              {r.role}
            </span>
          }
          name={r.name}
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <EntityMetricCard label="Company">
                      ...
                      {company ? <p className="text-caption font-medium text-muted-foreground">{company.industry} · {company.status}</p> : null}
                    </EntityMetricCard>
                    <EntityMetricCard label="Owner">
                      <p className="text-[14px] font-semibold text-foreground">{r.owner}</p>
                      <p className="text-caption font-medium text-muted-foreground">Account Executive</p>
                    </EntityMetricCard>
                  </div>
