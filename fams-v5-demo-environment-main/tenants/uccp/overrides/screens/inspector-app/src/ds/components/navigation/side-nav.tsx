      ...
      <TooltipTrigger asChild>
        <button
          ...
          style={{
            ...
            // Tokenized (was hardcoded FAMS blue rgba(0,114,214,x) — a token-only
            // violation that clashed on non-blue brands). Fallback keeps the FAMS
            // blue default; tenants override via --sidebar-item-accent[-idle].
            borderLeftColor: item.active
              ? 'var(--sidebar-item-accent, rgba(0,114,214,0.6))'
              : 'var(--sidebar-item-accent-idle, rgba(0,114,214,0.4))',
          }}
        >
...
/* One bracketed rail button — square rounded-[4px], white/20 (active → white),
   with an optional red indicator dot OR a numeric badge (e.g. unread count —
   additive, only renders when `badge` is set and `notificationDot` isn't). */
      ...
      <TooltipTrigger asChild>
        ...
        >
          ...
          {item.notificationDot ? (
            ...
            <span
              aria-hidden
              className="absolute -right-1 -top-1 flex h-[14px] min-w-[14px] items-center justify-center rounded-full px-[2px] text-[9px] font-bold"
              style={{ background: 'var(--status-error)', color: '#fff', border: '1.5px solid var(--sidebar)' }}
            >
