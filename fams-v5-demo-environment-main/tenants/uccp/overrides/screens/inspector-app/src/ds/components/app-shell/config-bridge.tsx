...
function AvatarDot({ label }: { label: string }) {
  ...
  return (
    <span
      className="inline-grid size-6 shrink-0 place-items-center rounded-full bg-primary text-caption font-bold text-primary-foreground"
      title={label}
      aria-label={label}
    >
...
export interface CellRenderContext {
  ...
  /** Detail surface (detail grid + sections): person refs render avatar + NAME
   *  (not the compact avatar-only dot used in cards/lists). */
  detail?: boolean;
}
...
export function renderCellValue(
  ...
  switch (name) {
    ...
    case 'AssigneeSelector':
      // Compact avatar-only in cards/lists; avatar + NAME on detail surfaces
      // (a bare "A" dot with no name reads as broken in a detail field).
      return ctx.detail ? <AvatarNameTag label={empty ? '' : label} /> : <AvatarDot label={empty ? '' : label} />;
    case 'AssigneeTag':
