...
function Initials({ name, color = 'var(--primary)' }: { name: string; color?: string }) {
  return <span className="grid size-8 shrink-0 place-items-center rounded-full text-caption font-semibold text-white" style={{ background: color }}>{name}</span>;
}
