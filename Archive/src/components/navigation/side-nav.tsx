...
export interface SideNavItem {
  ...
  active?: boolean;
  /**
   * Footer-item-only. Some Figma frames give Inbox a darker `black/20` chip
   * (vs. the other footer items' `white/20`) when it sits in the plain
   * footer group rather than its own top bracket. Ignored when `active`.
   */
  tone?: 'default' | 'dark';
  onClick?: () => void;
}
...
export interface SideNavProps {
  ...
  /**
   * Gap under the logo, before the app icons. Default `'sm'` (14px, Figma
   * 7134:2063 — this component's baked-in reference). Some pages' frames
   * (e.g. 31046:8494) show 28px instead — pass `'lg'` there rather than
   * changing the shared default for every consumer.
   */
  logoGap?: 'sm' | 'lg';
  className?: string;
}
    ...
    <TooltipProvider delayDuration={120}>
      <nav ref={navRef} aria-label="Primary" className={cn('flex h-full', className)}>
        ...
        >
          ...
          <div className={cn('flex flex-col items-center', logoGap === 'lg' ? 'gap-7' : 'gap-3.5')}>
            {logo ? (
