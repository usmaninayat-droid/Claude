...
import { ZonesView } from '../zones';
import { PoisView } from '../pois';
...
const REGISTRY: Record<ModuleType, ModuleTypeDef> = {
  ...
  pipeline: {
    ...
    render: (ctx) => {
      ...
      switch (ctx.activeTab.kind) {
        case 'list':
          return <PipelineListView data={data} onOpenDetail={ctx.openDetail} query={ctx.query} filters={ctx.filters} facetFilters={ctx.facetFilters} sort={ctx.sort} groupByCol={ctx.groupByCol} />;
        case 'calendar':
  ...
  'live-monitoring': {
    ...
    render: (ctx) => {
      ...
      const kind = ctx.activeTab.kind;
      const view = kind === 'list' || kind === 'map' ? kind : 'hybrid';
      return <LiveMonitoringView data={data} view={view} onOpenDetail={ctx.openDetail} />;
    },
  },
  ...
  zones: {
    ...
    label: 'Zones',
    render: (ctx) => {
      const data = ctx.module.data as ZonesModuleData | undefined;
      return data ? (
        <ZonesView
          zones={data.zones}
          center={data.center}
          zoom={data.zoom}
          labels={data.labels}
          onCreateZone={data.onCreateZone}
          onExport={data.onExport}
          onZoneClick={data.onZoneClick}
        />
      ) : (
        <PlaceholderView title={ctx.module.label} hint="Provide ZonesModuleData (zones + map centre) to render." />
      );
    },
  },
  ...
  pois: {
    icon: MarkerPin02,
    // Same single hybrid view as `zones` (T-040 items j/k — List View had no
    // value and the two module types must offer an identical view set).
    defaultViews: ['hybrid'],
    tabKind: 'view',
    label: 'POIs',
    render: (ctx) => {
      const data = ctx.module.data as PoisModuleData | undefined;
      return data ? (
        <PoisView
          pois={data.pois}
          icons={data.icons}
          resolveIcon={data.resolveIcon}
          center={data.center}
          zoom={data.zoom}
          labels={data.labels}
          onCreate={data.onCreate}
          onPoiClick={data.onPoiClick}
        />
      ) : (
        <PlaceholderView title={ctx.module.label} hint="Provide PoisModuleData (pois + icons) to render." />
      );
    },
  },
};
