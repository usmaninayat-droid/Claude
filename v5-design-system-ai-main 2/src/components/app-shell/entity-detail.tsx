...
export function EntityDetailRow({
  ...
  return (
    <div className={cn('flex items-center justify-between gap-3 py-2.5', borderBottom && 'border-b border-border')}>
      <p className="shrink-0 text-caption font-semibold text-muted-foreground">{label}</p>
      {typeof value === 'string' || typeof value === 'number' ? (
        <p className="min-w-0 truncate text-right text-caption font-semibold text-foreground">{value || '—'}</p>
      ) : (
...
export function EntityMetricCard({
  ...
  return (
    <div className="rounded-[8px] border border-border p-4">
      <p className="mb-1 text-caption font-semibold uppercase tracking-wide text-muted-foreground">{label}</p>
      {value != null ? <p className="text-h4 font-bold leading-tight text-foreground">{value}</p> : null}
      {sub ? <p className="text-caption font-medium text-muted-foreground">{sub}</p> : null}
...
export function EntityChartCard({
  ...
  return (
    <div className={cn('rounded-[8px] border border-border p-4', className)}>
      ...
      {subtitle ? <p className="mb-3 text-caption font-medium text-muted-foreground">{subtitle}</p> : null}
      {children}
    </div>
  );
}
  ...
  return (
    <div className={cn('flex items-center justify-between gap-3 py-2', borderBottom && 'border-b border-border')}>
      <div className="flex min-w-0 items-center gap-2.5">
        {leading}
        <p className="truncate text-caption font-medium text-foreground">{title}</p>
      </div>
...
export function EntityDetail({
  ...
  // T-081/T-095 backport: a tab count that outgrows the panel's fixed width
  // (12+ full-word labels) must scroll horizontally, never wrap or clip off
  // the edge — the tablist itself is the scroll container (hidden-scrollbar
  // idiom, matching live-monitoring-view.tsx's/pipeline-right-panel.tsx's own
  // overflow-x tab rows), and the active tab scrolls into view whenever it
  // changes (covers both a click AND an external `defaultTabId` change).
    ...
    <div className={cn('flex h-full min-h-0 items-stretch', className)}>
      ...
      <aside className="w-[320px] shrink-0 overflow-y-auto border-r border-border bg-muted/40">
        <div className="flex flex-col gap-6 p-7">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-h5 font-bold leading-snug text-foreground">{name}</p>
            ...
            <div className="relative mt-2 flex flex-wrap items-center gap-1.5">
              ...
              {(tags ?? []).map((t) => (
                <span
                  key={t.id}
                  className="group flex items-center gap-1 rounded-[2px] border border-primary bg-secondary py-1 pl-2 pr-1.5 text-caption font-semibold text-primary"
                >
                  {t.label}
                  {onRemoveTag ? (
                    <button
                ...
                <Popover open={tagOpen} onOpenChange={(o) => { setTagOpen(o); if (!o) setTagQuery(''); }}>
                  <PopoverTrigger asChild>
                    <button
                      type="button"
                      aria-label="Add tag"
                      className="grid size-7 place-items-center rounded-[2px] bg-muted text-body-md font-semibold text-foreground transition-colors hover:bg-border"
                    >
                      +
                    </button>
                  </PopoverTrigger>
                  <PopoverContent align="start" className="w-60 p-2">
                    <div className="relative mb-1">
                      <Search size={14} className="pointer-events-none absolute left-2 top-1/2 z-10 -translate-y-1/2 text-muted-foreground" />
                      <input
                        autoFocus
                        value={tagQuery}
                        ...
                        className="h-8 w-full rounded-md bg-muted/60 pl-7 pr-2 text-body-sm outline-none placeholder:text-muted-foreground"
                      />
                    </div>
          ...
          {info?.length ? (
            <div className="flex flex-col">
              <p className="mb-3 text-body-sm font-semibold text-muted-foreground">{infoTitle}</p>
              {info.map((row, i) => (
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden bg-card">
        <div className="shrink-0 px-7 pt-5">
          <div
            className="flex items-center gap-8 overflow-x-auto whitespace-nowrap border-b border-border [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
            role="tablist"
          >
            {tabs.map((t) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'shrink-0 pb-2.5 text-body-sm outline-none transition-colors',
                    active
