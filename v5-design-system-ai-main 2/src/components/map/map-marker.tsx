...
/** Canonical asset-state → colour-token map — shared with map widgets/legends. */
export const ASSET_STATE_COLOR: Record<AssetMarkerState, string> = {
...
export function AssetMarker({
  ...
  const color = ASSET_STATE_COLOR[state];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {badge ? (
          <span
            className="absolute -right-1 -top-1 flex size-4 items-center justify-center rounded-full text-white"
            style={{ background: color, boxShadow: '0 0 0 1.5px white', fontSize: 8 }}
          >
            {badge}
          </span>
        ) : null}
