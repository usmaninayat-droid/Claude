...
> **Autonomous team (self-driving).** Before non-trivial work, read `.claude/team/`
> — `workspace/backlog.md` + `tickets.md` (what to do) and `memory/` (what we learned) —
> and append to them after. The brain is `/conductor tick` (state-driven; picks the top
> item itself). Loop engine + gates: `docs/AGENT-TEAM-ARCHITECTURE.md`. Never grind past
> the 3-iteration cap — escalate. Cheap gates (`scripts/team/gates/*`) before model reviews.
